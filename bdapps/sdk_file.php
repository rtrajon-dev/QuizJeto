...
class WebApi extends Core{
      ...
      private   $requestId ,
                ...
                $id,
                $conn;
    public      $url = "https://developer.bdapps.com/subscription/activate";


        public function __construct(){
            //$this->conn = new mysqli($this->servername, $this->username, $this->password,$this->dbname);
            $this->requestId = rand(100000000000000000,999999999999999999);
            $this->requestTime = $this->getTime();
            $this->serviceProviderAppId = $this->getServiceProviderAppId();
            // Create connection
        ...
        public function getServiceProviderAppId(){
            return "DSAPP_000003";
        }
        
        public function getAppandSubscriber($address){
                $this->subscriberId = $address;
                $myfile = fopen("appid.txt", "r") or die("Unable to open file!");
                $app =  fgets($myfile);
                $app = trim(preg_replace('/\s\s+/', '', $app));
                fclose($myfile);
                $this->applicationId = $app;
                $this->requestSignature = hash('sha256',$this->applicationId.$this->requestTime."8c899b6b56e6855605ea61be994012e7");
        }
