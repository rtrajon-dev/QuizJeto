...
class SMSSender  extends Core{
	...
	*/		
	...
	private function handleResponse($jsonResponse){
		...
		$a = json_decode($jsonResponse);
		$b = json_decode(json_encode($a->destinationResponses));
		$b = $b[0];
		$statusCode = $b->statusCode;
		//$statusDetail = $b[0]->statusDetail;
	    //file_put_contents("handleresponse.txt",json_encode($a->destinationAddresses));
 		//$statusCode = $jsonResponse->statusCode;
		//$statusDetail = $jsonResponse->statusDetail;
		//file_put_contents('status_code.txt',$status_code);
